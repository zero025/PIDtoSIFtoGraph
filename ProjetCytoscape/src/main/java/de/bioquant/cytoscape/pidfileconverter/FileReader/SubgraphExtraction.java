...
public class SubgraphExtraction {
	...
	public SubgraphExtraction(MainFrame mainframe) {
		JOptionPane
		.showMessageDialog(new JFrame(), "Beginning constructor", "Success", JOptionPane.INFORMATION_MESSAGE);
		...
		inputgenetargetfilepath = mainframe.getGenetargettextfield().getText();
		inputsigmolsourcefilepath = mainframe.getSigmolsourcetextfield().getText();
		inputsigmoltargetfilepath = mainframe.getSigmoltargettextfield().getText();
	}
	...
	/**
	 * Read the cytoidsourcearea of class MainFrame and reads it according to CytoIDs, checks if present in CyNetwork, before putting the IDs into an
	...
	public void readCytoSourceText() {
		if (!inputcytosourcetext.trim().equals("")) // if the text area is not empty
		{
			...
			try {
				reader = new BufferedReader(new StringReader(inputcytosourcetext));
				String l;
				...
				while ((l = reader.readLine()) != null) {
					...
					if (cynetwork == null || cynodelist.isEmpty()) {
						...
						for (int i = 0; i < cynodelist.size(); i++) {
							...
							if (cynodelist.get(i).getIdentifier().equals(l.trim())) {
				...
				System.out.println("Nodes in the cyto ID source text not recognized:");
	...
	/**
	 * Read the cytoidtargetarea of class MainFrame and reads it according to CytoIDs, checks if present in CyNetwork, before putting the IDs into an
	 * ArrayList of type CyNode, which is to undergo subgraph extraction.
	 */
	public void readCytoTargetText() {
		if (!inputcytotargettext.trim().equals("")) {
			...
			ArrayList<String> temporarylist = new ArrayList<String>(); // temporary list for nodes NOT in the graph
			try {
				reader = new BufferedReader(new StringReader(inputcytotargettext));
				String l;
				// get the network view object and its nodelists
				CyNetwork cynetwork = Cytoscape.getCurrentNetwork();
				List<CyNode> cynodelist = Cytoscape.getCyNodesList();
				...
				System.out.println("Nodes from the cyto ID target text not recognized:");
				for (int i = 0; i < temporarylist.size(); i++) {
					System.out.println(temporarylist.get(i));
				}
			}
	...
	public void readGeneSourceFile() {
		if (!inputgenesourcefilepath.trim().equals("")) {
			BufferedReader reader = null;
			ArrayList<String> temporarylist = new ArrayList<String>(); // temporary list for nodes NOT in the graph
			try {
				reader = new BufferedReader(new FileReader(inputgenesourcefilepath));
				String l;
				// get the network view object and its nodelists
				CyNetwork cynetwork = Cytoscape.getCurrentNetwork();
				List<CyNode> cynodelist = Cytoscape.getCyNodesList();
				while ((l = reader.readLine()) != null) {
					...
					if (cynetwork == null || cynodelist.isEmpty()) {
						...
						for (int i = 0; i < cynodelist.size(); i++) {
							// if the read line l is same as one of the list of nodes, add that node to the list to be subgraphed!
							if (cynodelist.get(i).getIdentifier().equals(l.trim())) {
								if (!cytosourcesubgraph.contains(cynodelist.get(i))) {
									cytosourcesubgraph.add(cynodelist.get(i));
									temporarylist.remove(l.trim()); // if the line is not present in the list of nodes, delete it from temporary list!
								}
							}
				...
				JOptionPane.showMessageDialog(new JFrame(), "Please check the gene source file! Click ? for help.",
						"Caution!", JOptionPane.ERROR_MESSAGE);
				return;
			} finally {
				System.out.println("Nodes from the gene source not recognized:");
				for (int i = 0; i < temporarylist.size(); i++) {
					System.out.println(temporarylist.get(i));
				}
	...
	/**
	 * This method reads the gene target file and checks if the input nodes are present in the cynodelist. if they are present, check if the node
	 * comes from a translation event. If yes, check if it is a protein family or complex, if no, then add it into the arraylist cytotargetsubgraph
	 */
	public void readGeneTargetFile(String sifpath, String nodetypefilepath) {
		if (!inputgenetargetfilepath.trim().equals("")) {
			...
			ArrayList<String> temporarylist = new ArrayList<String>(); // temporary list for nodes NOT in the graph
			try {
				reader = new BufferedReader(new FileReader(inputgenetargetfilepath));
				String l;
				// get the network view object and its nodelists
				CyNetwork cynetwork = Cytoscape.getCurrentNetwork();
				List<CyNode> cynodelist = Cytoscape.getCyNodesList();
				while ((l = reader.readLine()) != null) {
					...
					if (cynetwork == null || cynodelist.isEmpty()) {
						...
						if (isComingFromTranslation(l.trim(), sifpath, nodetypefilepath)) // if l comes from a translation event
						{
							for (int i = 0; i < cynodelist.size(); i++) {
								if (!cynodelist.get(i).getIdentifier().contains(":")) // if cytoID doesnt contain ":" i.e. not family or complex
								{
									if (cynodelist.get(i).getIdentifier().contains(l.trim())) // if cytoID contains l, i.e. P10291@nucleus is possible
									{
										if (!cytotargetsubgraph.contains(cynodelist.get(i))) // then add the cytoid to the target subgraph list
										{
											cytotargetsubgraph.add(cynodelist.get(i));
											temporarylist.remove(l.trim()); // if the line is not present in the list of nodes, delete it from
											// temporary list!
										}
									}
								}
				...
				JOptionPane.showMessageDialog(new JFrame(), "Please check the gene target file! Click ? for help.",
						"Caution!", JOptionPane.ERROR_MESSAGE);
				return;
			} finally {
				System.out.println("Nodes from the gene target not recognized:");
				for (int i = 0; i < temporarylist.size(); i++) {
					System.out.println(temporarylist.get(i));
				}
				// have to close the reader, else wasting of resources
	...
	/**
	 * This method reads the file and checks if the checkbox is ticked. If yes, the method will search also in protein families and complexes. If no,
	 * the method will only search for nodes of proteins. In both ways, the method adds positive searches into the cytosourcesubgraph arraylist.
	 */
	public void readSigmolSourceFile() {
		if (!inputsigmolsourcefilepath.trim().equals("")) {
			...
			ArrayList<String> temporarylist = new ArrayList<String>(); // temporary list for nodes NOT in the graph
			try {
				reader = new BufferedReader(new FileReader(inputsigmolsourcefilepath));
				String l;
				// get the network view object and its nodelists
				CyNetwork cynetwork = Cytoscape.getCurrentNetwork();
				List<CyNode> cynodelist = Cytoscape.getCyNodesList();
				while ((l = reader.readLine()) != null) {
					...
					if (cynetwork == null || cynodelist.isEmpty()) {
						...
						for (int i = 0; i < cynodelist.size(); i++) {
							if (mainframe.isIncludeComplexesChecked()) // if the checkbox is ticked
							{
								if (cynodelist.get(i).getIdentifier().contains(l.trim())) // see if the string of UniProt is present in the CytoID
								{
									if (!cytosourcesubgraph.contains(cynodelist.get(i))) // add it to the subgraph list
									{
										cytosourcesubgraph.add(cynodelist.get(i));
										temporarylist.remove(l.trim()); // if the line is not present in the list of nodes, delete it from temporary
										// list!
									}
								}
							} else if (!mainframe.isIncludeComplexesChecked()) // else if the checkbox is not ticked
							{
								if (!cynodelist.get(i).getIdentifier().contains(":")) // check if the CytoID is NOT a family/complex (both contain
									// ":")
								{
									if (cynodelist.get(i).getIdentifier().contains(l.trim())) // if so, add the text to the subgraph list
									{
										if (!cytosourcesubgraph.contains(cynodelist.get(i))) {
											cytosourcesubgraph.add(cynodelist.get(i));
											temporarylist.remove(l.trim()); // if the line is not present in the list of nodes, delete it from
				...
				JOptionPane.showMessageDialog(new JFrame(),
						"Please check the signalling molecules source file! Click ? for help.", "Caution!",
						JOptionPane.ERROR_MESSAGE);
				return;
			} finally {
				System.out.println("Nodes from the signalling molecules source not recognized:");
				for (int i = 0; i < temporarylist.size(); i++) {
					System.out.println(temporarylist.get(i));
				}
				// have to close the reader, else wasting of resources
	...
	/**
	 * This method reads the file and checks if the checkbox is ticked. If yes, the method will search also in protein families and complexes. If no,
	 * the method will only search for nodes of proteins. In both ways, the method adds positive searches into the cytotargetsubgraph arraylist.
	 */
	public void readSigmolTargetFile() {
		if (!inputsigmoltargetfilepath.trim().equals("")) {
			...
			ArrayList<String> temporarylist = new ArrayList<String>(); // temporary list for nodes NOT in the graph
			try {
				reader = new BufferedReader(new FileReader(inputsigmoltargetfilepath));
				String l;
				// get the network view object and its nodelists
				CyNetwork cynetwork = Cytoscape.getCurrentNetwork();
				List<CyNode> cynodelist = Cytoscape.getCyNodesList();
					...
					if (cynetwork == null || cynodelist.isEmpty()) {
						...
						for (int i = 0; i < cynodelist.size(); i++) {
							...
							if (mainframe.isIncludeComplexesChecked()) // if the checkbox is ticked
							{
								if (cynodelist.get(i).getIdentifier().contains(l.trim())) // see if the string of UniProt is present in the CytoID
								{
									if (!cytotargetsubgraph.contains(cynodelist.get(i))) // add it to the subgraph list
									{
										cytotargetsubgraph.add(cynodelist.get(i));
										temporarylist.remove(l.trim()); // if the line is not present in the list of nodes, delete it from temporary
										// list!
							...
							} else if (!mainframe.isIncludeComplexesChecked()) // else if the checkbox is not ticked
							{
									...
									// ":")
								{
									if (cynodelist.get(i).getIdentifier().contains(l.trim())) // if so, add the text to the subgraph list
									{
										if (!cytotargetsubgraph.contains(cynodelist.get(i))) {
											cytotargetsubgraph.add(cynodelist.get(i));
											temporarylist.remove(l.trim()); // if the line is not present in the list of nodes, delete it from
											// temporary list!
										}
				...
				JOptionPane.showMessageDialog(new JFrame(),
						"Please check the signalling molecules file! Click ? for help.", "Caution!",
						JOptionPane.ERROR_MESSAGE);
				return;
			} finally {
				System.out.println("Nodes from the signalling molecules target not recognized:");
				for (int i = 0; i < temporarylist.size(); i++) {
					System.out.println(temporarylist.get(i));
				}
				// have to close the reader, else wasting of resources
	...
	/**
	 * Method reads the arraylist of IDs and draws the subgraph using the shortest path method. Checks both cytosourcesubgraph and cytotargetsubgraph
	 * for a shortest path between the source and target nodes.
	 * 
	 * @author Hadi Kang
	...
	/**
	 * This method reads the input SIF file and compares its contents with the list of nodes to be deleted. Then a new SIF would be created, depending
	...
	public void SIFreaderAndNewCreator(String pathname) throws IOException {
		...
		try {
			...
			String filetobewritten = temporarypath[0].concat(Controller.getSubgraphed() + ".sif");
			...
			while ((line = reader.readLine()) != null) {
				...
				if (line.contains("\t")) {
					...
					// if the list of nodes to be kept contains the IDcyto read from the SIF, then pass it to the new SIF by printing it
					if (nodeIDtobekept.contains(node1) && nodeIDtobekept.contains(node2)) {
						writer.println(line);
	...
	/**
	 * This method checks the sif and the nodetype.na files to see if the node is present, and if the node origins from a transcription event
	 * 
	 * @param nodename
	 ...
	 * @throws IOException
	 */
	public boolean isComingFromTranslation(String nodename, String sifpath, String nodetypefilepath) throws IOException {
		boolean value = false;
		BufferedReader reader = null;
		try {
					...
					if (line.contains("\t")) {
						...
						if (!node2.contains(":")) // if there is no ":", i.e. not family nor complex
						{
							if (node2.contains(nodename)) {
								if (isPrecursorNodeTranscription(node1, nodetypefilepath)) {
									value = true;
									return value;
								}
							} else {
	 ...
	 * @throws IOException
	 */
	public boolean isPrecursorNodeTranscription(String precursornode, String nodetypefilepath) throws IOException {
		boolean value = false;
		BufferedReader reader = null;
		try {
			...
			String line;
			while ((line = reader.readLine()) != null) {
				// if the line in the NodeType file contains the node referring to transcription
				if (line.contains(precursornode) && line.contains("transcription")) {
					value = true;
					return value;
				} else {
					value = false;
	...
	public void colourThisNode(CyNode node, String nodecolourRGB) {
		...
		nodeAtts.setAttribute(node.getIdentifier(), "node.fillColor", nodecolourRGB);
