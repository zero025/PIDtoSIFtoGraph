...
public class IlluminaRegexReader {
	...
	/**
	 * This method reads one illumina file and assigns a value (barcode) to a key (GeneID) and puts them into a hashmap, 1 or 0 from
	 * barcodeFilter(String line)
	 * 
	 * @param barcodefilepath
	 * @throws IOException
	 */
	public static void fileReader(String barcodefilepath, String inputCondition1Text, String inputCondition2Text)
			throws IOException {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (splittedString.length > 107) {
					...
					// The conditions are from column "C" to "CT" : from 3 to 26*2+20=72. We keep the ".mean" and ".p" values : first and third values
					// of each set of four columns
					for (int i = 2; i < 72; i += 4) {
						if ((inputCondition1Text.equals("") && inputCondition1Text.equals(""))
	...
	/**
	 * This method checks if the defaultID is present in the hashmap uniprottogeneid_fullhashmap and then crosschecking in the barcode1hashmap and
	 * barcode2hashmap, before concluding which idcyto to delete, and returning a boolean of true if something is deleted, else false!
	 * 
	 * @param defaultid
	 */
	...
	/**
	 * This method reads the barcode hashmaps and decides what to do with the barcode lines. If not expressed (0), delete the node corresponding to
	 * the illuminaID. If expressed (1), leave it
	 * 
	 * @param illuminaID
	 *            the illuminaID
	 * @param value
	...
	public static void compareConditions() {
		...
		for (Map.Entry<String, String> entryFromProteinFamilyHashmap : AffymetrixRegexReader.getProteinfamilyhashmap()
				.entrySet()) {
		...
		JOptionPane.showMessageDialog(new JFrame(), "The graph view has been filtered and updated. " + counter
				+ " nodes have been removed.", "Filtered", JOptionPane.PLAIN_MESSAGE);
		counter = 0;
	}
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (line.contains("\t")) {
					...
					if (!(nodeIDtobedeleted.contains(node1) || nodeIDtobedeleted.contains(node2))) // if list of nodes does not contain IDcyto to be
																									// deleted, then pass it to the new SIF by
																									// printing it
					{
						writer.println(line);
					}
	...
	/**
	 * This method clears the barcode hashmaps and the ArrayList nodeIDtobedeleted. If the Hashmaps are not cleared, then the next filtering might be
	 * erroneous!
	 */
